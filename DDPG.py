"""
We refer to the Reinforcement Learning method of
Deep Deterministic Policy Gradient (DDPG) on github.
...
def init_env(env,MEMORY_CAPACITY):
    #####################  parameters  ####################
    ...
    sess = tf.Session()
    print('Initilize Actor Network...')
    # Create actor and critic.
    # They are actually connected to each other, details can be seen in tensorboard or in this picture:
    actor = Actor(sess, action_dim, action_bound, LR_A, REPLACEMENT,S,S_)
    print('Initilize Critic Network...')
    critic = Critic(sess, state_dim, action_dim, LR_C, GAMMA, REPLACEMENT, actor.a, actor.a_,S,R,S_)
    print('Build Actor-Critic...')
    actor.add_grad_to_graph(critic.a_grads)
    print('Complete...')
    sess.run(tf.global_variables_initializer())

    print('Initilize Memory Space...')
    M = Memory(MEMORY_CAPACITY, dims=2 * state_dim + action_dim + 1)

    #if OUTPUT_GRAPH:
    #    tf.summary.FileWriter("logs/", sess.graph)
    return actor, critic, M
